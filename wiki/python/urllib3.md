...
| CVE-2025-66471 / GHSA-2xpw-w6gg-jr37 | High | The streaming API could fully decode a small amount of highly compressed data in a single operation, defeating chunk-size expectations and causing client-side resource exhaustion (high CPU + massive memory allocation; CWE-409). Advisory remediation also calls out upgrading Brotli / brotlicffi to 1.2.0 if Brotli encoding support is installed. | 2.6.0 | https://github.com/urllib3/urllib3/security/advisories/GHSA-2xpw-w6gg-jr37 |
...
- The public 2025-2026 decompression-bomb advisory trail also highlights a **transitive-dependency nuance**: GHSA-2xpw-w6gg-jr37 notes that if Brotli encoding support is present, users should also upgrade `Brotli` / `brotlicffi` to `>= 1.2.0` (the patched urllib3 versions enforce this via the `urllib3[brotli]` extra).
...
---
*Last updated: 2026-04-29 | Sources: 5 (OSV package query, GitHub Advisory Database / GHSA pages, CVE aliases via OSV, upstream CHANGES.rst, repository SECURITY.md; reviewed GHSA-2xpw-w6gg-jr37 remediation note re: Brotli/brotlicffi 1.2.0)*
