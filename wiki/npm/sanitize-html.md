...
- The newest record is especially actionable: OSV and GitHub's advisory describe an **`allowedTags` enforcement bypass** in `sanitize-html` `2.17.2` (fixed in `2.17.3`) that specifically applies when callers allow `option` or `textarea` (i.e., tags in `nonTextTagsArray`) and rely on entity-encoded text inside those elements being safely handled. This is a configuration-dependent issue (not triggered by default `allowedTags`), but it can defeat the core safety guarantee in affected deployments.
